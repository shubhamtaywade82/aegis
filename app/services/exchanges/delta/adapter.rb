  ...
  module Delta
    class Adapter < BaseAdapter
      ...
      def latest_price(symbol)
        resolved_symbol = SymbolRegistry.resolve(:delta, symbol)
        BigDecimal("100.0")
      end

      def place_order(order_request)
        resolved_symbol = SymbolRegistry.resolve(:delta, order_request.symbol)
        precision = PrecisionRegistry.for(:delta, order_request.symbol)
        ...
        OrderResponse.new(
          exchange_order_id: "delta_#{SecureRandom.hex(6)}",
      ...
      def cancel_order(symbol:, order_id:)
        resolved_symbol = SymbolRegistry.resolve(:delta, symbol)
        {
          ...
          client_order_id: "delta_#{SecureRandom.hex(6)}",
          status: :cancelled
        }
      end
